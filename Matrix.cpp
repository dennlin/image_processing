...
const int* Matrix_at(const Matrix* mat, int row, int column) {
  return (const int*)(&(mat->data[mat->width*row+column])); 
}
...
void Matrix_fill_border(Matrix* mat, int value) {
  for(int i=0; i<mat->width; ++i){
    mat->data[i]=value; // top row
  }

  for(int i=0; i<mat->width; ++i){
    int *element = Matrix_at(mat, mat->height-1, i); // bottom row 
    (*element)=value;

  }
   
   for(int i=1; i<mat->height-1; ++i){
    int *element = Matrix_at(mat, i, 0); // left column 
    (*element)=value;
  }
  
  for(int i=1; i<mat->height-1; ++i){
    int *element = Matrix_at(mat, i, mat->width-1); // right column 
    (*element)=value;
  }
...
int Matrix_column_of_min_value_in_row(const Matrix* mat, int row, int column_start, int column_end) {
  int *firstelement = (int*)Matrix_at(mat, row, column_start);
...
int *firstelement = (int*)Matrix_at(mat, row, column_start);
  int size = column_end-column_start;
  int min = firstelement[0];
  for(int i=0; i<size; ++i){
    int current = firstelement[i];
    if(current<min){
      min=current;
    }
  }
