...
void Image_init(Image* img, std::istream& is) {
  ...
  string word;
    ...
    is >> newline >> img->width >> img->height >> newline;
    ...
    while (!is.eof()) {
        is >> red[index] >> green[index] >> blue[index];
        ++index;
    }
}
...
void Image_print(const Image* img, std::ostream& os) {
  using namespace std;
  string newLine = "\n";
  os << "P3" << newline << img->width << " " << img->height << "255" << newLine;
  
  int *red = (int*)Matrix_at(&img->red_channel,0,0);
  int *green = (int*)Matrix_at(&img->green_channel,0,0);
  int *blue = (int*)Matrix_at(&img->blue_channel,0,0);
...
void Image_print(const Image* img, std::ostream& os) {
  using namespace std;
  string newLine = "\n";
  os << "P3" << newLine << img->width << " " << img->height << newLine << "255" << newLine;
